// File: src/analytics_logger_node.cpp
...
struct CycleData {
  std::string model;         // e.g., "Red_3"
  std::string color;         // "red"/"blue" (from detection)
  std::string size;          // "small"/"medium"/"large" (if provided)
  std::string detection_ts;  // "YYYY-MM-DD HH:MM:SS"
  std::string pick_ts;
  std::string place_ts;
...
public:
  ...
  : Node("analytics_logger_node")
  {
    ...
    db_path_ = this->declare_parameter<std::string>(
      "db_path", std::string(std::getenv("HOME") ? std::getenv("HOME") : ".") + "/parol6/pipeline_analytics.db"
    );
    csv_path_ = this->declare_parameter<std::string>(
      "csv_path", std::string(std::getenv("HOME") ? std::getenv("HOME") : ".") + "/parol6/pipeline_analytics.csv"
    );
    ...
    // Subscriptions
    // Expect JSON on /analytics/event with shape:
    // { "event":"detection|pick|place",
    //   "model":"Red_1",
    //   "color":"red",         // detection only
    //   "size":"small",        // optional
    //   "timestamp":"YYYY-MM-DD HH:MM:SS" // optional; if absent, node will use current time
    // }
    sub_events_ = this->create_subscription<std_msgs::msg::String>(
      "/analytics/event", rclcpp::QoS(50),
      std::bind(&AnalyticsLoggerNode::onEvent, this, std::placeholders::_1)
    );

    // Live feed publisher (optional to monitor)
    ...
    // Clear table service (handy in sim)
...
private:
  ...
  // Upsert-like behavior: we INSERT a new row at detection, then UPDATE on pick/place.
  // We identify the row by (sim_start_ts, model).
  ...
  void onEvent(const std_msgs::msg::String::SharedPtr msg) {
    // Accepts JSON events; robust to missing optional fields
    // Example messages are shown later in "Publish from your pipeline" section.
    json j;
    try {
    ...
    if (event == "detection") {
      ...
      d.place_ts = ts;
      // Compute duration if we have detection_ts
      if (!d.detection_ts.empty()) {
        // naive: compute in seconds using system time only when all came from node time;
        // here we just skip exact diff parsing and leave 0 if missing detection.
        // For reliable duration, pipeline should send consistent timestamps or let this node timestamp events.
        // Since we timestamp here on absence, we have both, so it's fine.
  ...
  // Live JSON to /analytics/live and console
  ...
  // Very simple duration estimator between two "YYYY-MM-DD HH:MM:SS" local strings.
  // (Avoids bringing in heavy time parsing; acceptable for sim analytics.)
  ...
  void onExportCsv(const std::shared_ptr<std_srvs::srv::Trigger::Request>,
    ...
    try {
      // Write CSV by querying DB and streaming to file
